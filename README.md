...
### Building type guards with documentation
...
export const addTree = (
  ...
  req.on("end", () => {
    try {
      const { id, species, age, location } = JSON.parse(body);
...
export const server = http.createServer((req, res) => {
  if (req.method === "GET" && req.url?.startsWith("/trees/")) {
    getTreeById(req, res);
  } else if (req.method === "POST" && req.url === "/trees") {
    addTree(req, res);
  } else if (req.method === "PUT" && req.url?.startsWith("/trees/")) {
    updateTree(req, res);
  } else {
